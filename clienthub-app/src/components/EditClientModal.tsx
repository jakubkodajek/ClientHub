...
interface EditClientModalProps {
  client: Client;
  onClose: () => void;
  onUpdate: (data: { name: string; description: string; imageUrl: string }) => Promise<void>;
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 bg-surface-container-lowest dark:bg-dark-surface-container-lowest">
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-4 py-3 bg-surface-container-high dark:bg-dark-surface-container-high rounded-xl border-none text-on-surface dark:text-dark-on-surface focus:ring-2 focus:ring-primary/30 outline-none text-sm resize-none"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-on-surface dark:text-dark-on-surface mb-2">
